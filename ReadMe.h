...
The toy programming language to develop is a variation (and oversimplification) of assembly. Our version of the language will handle a drastically reduced number of lexical elements, which are described as follows: ..* INTEGER A sequence of digits. ..* REGISTER The symbol # followed by only one of the following letters: A, B, C, and D. ..* OPERATION One of the following reserved words: SUM, SUB, MUL, and DIV. ..* ASSIGNMENT The reserved word MOV. ..* EOF The symbol ;.
...
An OPERATION (SUM, SUB, MUL, and DIV) must be followed by two registers. For example: SUM #A #B. It is not allowed for an operation to involve an INTEGER directly (it must be assigned to a register to be used).
An ASSIGNMENT (MOV) can take an INTEGER or a REGISTER as first operand. However, the second operand must be a REGISTER. For example: MOVE 12 #A or MOV #B #D.
Only at the end of the file, the symbol ; must be present. This is to indicate the end of the file.
For example, the following would be a valid program for this language: MOV 5 #A MOV 10 #B SUM #A #B MOV #B #C MUL #C #A MOV #C #D MOV 2 #A SUB #D #A ; Please consider that your system will read the input from a text file.
...
Deliverables
Implement a program that integrates the scanner and parser for the language described Prepare a ZIP file that contains your codes and a brief PDF document (at most two pages) with the DFA and grammar designed, and submit it to Canvas. If you worked as part of a team, only one of the teammates should submit the solution. Please, do not submit other formats but ZIP.
